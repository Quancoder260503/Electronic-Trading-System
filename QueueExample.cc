...
auto ConsumeFunction(LockFreeQueue<MyStruct>* lock_free_queue) { 
  ...
  while(lock_free_queue->size()) { 
   ...
   std::cout << "consumeFunction read elem : " 
             << elem->d[0] << " " << elem->d[1] << " " << elem->d[2] << " lfq-size : " << lock_free_queue->size() << std::endl; 
// std::flush(std::cout); 
   std::this_thread::sleep_for(5s);        
  }
...
int main(void) {
 ...
 for(int i = 0; i < 50; i++) { 
  ...
  std::cout << "main constructed element : " << curr.d[0] << " " << curr.d[1] << " " << curr.d[2] << " lfq-size : " << lock_free_queue.size() << std::endl;  
//std::flush(std::cout); 
  using namespace std::literals::chrono_literals; 
