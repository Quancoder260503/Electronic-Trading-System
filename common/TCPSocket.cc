...
auto TCPSocket::sendAndRecv() noexcept -> bool {
  ...
  iovec iov{recv_buffer.data() + next_recv_valid_index, TCPBufferSize - next_recv_valid_index};
  msghdr msg{&socket_attribute, sizeof(socket_attribute), &iov, 1, ctrl, sizeof(ctrl), 0};
  ...
  if (read_size > 0) {
    ...
    logger.log("%:% %() % read socket:% len:% utime:% ktime:% diff:%\n", __FILE__, __LINE__,
               __FUNCTION__, Common::getCurrentTimeStr(&time_str), socket_fd, next_recv_valid_index,
  ...
  if (next_send_valid_index > 0) {
    ...
    const auto n =
        ::send(socket_fd, send_buffer.data(), next_send_valid_index, MSG_DONTWAIT | MSG_NOSIGNAL);
    logger.log("%:% %() % send socket:% len:%\n", __FILE__, __LINE__, __FUNCTION__,
               Common::getCurrentTimeStr(&time_str), socket_fd, n);
  }
