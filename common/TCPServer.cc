...
auto TCPServer::poll() noexcept -> void {
    ...
    if (event.events & EPOLLIN) {
      if (socket == &listener_socket) {
        logger.log("%:% %() % EPOLLIN listener_socket:%\n", __FILE__, __LINE__, __FUNCTION__,
                   Common::getCurrentTimeStr(&time_str), socket->socket_fd);
  ...
  while (have_new_connection) {
    ...
    int fd = accept(listener_socket.socket_fd, reinterpret_cast<sockaddr *>(&addr), &addr_len);
    if (fd == -1) {
      break;
    }
    ...
    logger.log("%:% %() % accepted socket:%\n", __FILE__, __LINE__, __FUNCTION__,
               Common::getCurrentTimeStr(&time_str), fd);
