...
MatchingEngineOrderBook::MatchingEngineOrderBook(TickerID ticker_id_, Logger *logger_,
...
auto MatchingEngineOrderBook::match(TickerID ticker_id, ClientID client_id, Side side,
  ...
  client_response = {ClientResponseType::FILLED, client_id, ticker_id,  client_order_id,
  ...
  if (!order->quantity) {
    market_update = {
        MarketUpdateType::CANCEL, order->market_order_id, ticker_id, order->side, order->price,
        order_quantity,           PRIORITY_INVALID};
    matching_engine->sendMarketUpdate(&market_update);
    removeOrder(order);
  } else {
    market_update = {
        MarketUpdateType::MODIFY, order->market_order_id, ticker_id, order->side, order->price,
        order->quantity,          order->priority};
    matching_engine->sendMarketUpdate(&market_update);
  }
}

auto MatchingEngineOrderBook::checkForMatch(OrderID client_id, OrderID client_order_id,
                                            TickerID ticker_id, Side side, Price price,
                                            Quantity quantity,
                                            Quantity new_market_order_id) noexcept {
...
auto MatchingEngineOrderBook::add(ClientID client_id, OrderID client_order_id, TickerID ticker_id,
  ...
  client_response = {ClientResponseType::ACCEPTED,
                     ...
                     quantity};
  matching_engine->sendClientResponse(&client_response);
  const auto leaves_quantity = checkForMatch(client_id, client_order_id, ticker_id, side, price,
  ...
  if (LIKELY(leaves_quantity)) {
    ...
    auto order = order_pool.allocate(ticker_id, client_id, client_order_id, new_market_order_id,
                                     side, price, leaves_quantity, nullptr, nullptr);
