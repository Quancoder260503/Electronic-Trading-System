...
int main(void) {
 ...
 auto tcpServerRecvCallback = [&](TCPSocket *socket, Nanos rx_time) noexcept -> void { 
 ...
 auto tcpClientRecvCCallback = [&](TCPSocket *socket, Nanos rx_time) noexcept -> void { 
    ...
    logger.log("TCPSocket::defaultRecvCallback() socket:% len:% rx:% msg:%\n",
                socket->socket_fd, socket->next_recv_valid_index, rx_time, recv_msg);
 }; 
