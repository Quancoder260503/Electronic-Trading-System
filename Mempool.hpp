  ...
  class MemPool final { 
   public: 
    ...
    T *allocate(Args ... args) noexcept { 
      ...
      ASSERT(object_block->is_free, "Expected free block at index : " + std::to_string(next_free_index_)); 
      T *ret = &(object_block->object); 
      ret = new(ret) T(args ...); // Construct of new object of type T on the memory address of the current one (overwrite it)
    ...
    auto deallocate(const T *elem) noexcept { 
      ...
      ASSERT(!store_[elem_index].is_free, "Expected in use Object block at index : " + std::to_string(elem_index)); 
